...
import pytest
from coworker.compaction import DEFAULT_CAP_TOKENS, DEFAULT_CONTEXT_WINDOW, trigger_tokens
...
# Marlo：Qumge 网关的默认模型没有核实过的窗口。不编一个数：它压缩走的是
# DEFAULT_CAP_TOKENS（60k，feat/compaction-cap），远低于任何现役模型的窗口，
# 所以 128k 的猜测在这里不起作用。
_KNOWN_UNVERIFIED_RECOMMENDATIONS = {
    "minimax:MiniMax-M2.5",
    "meta:muse-spark-1.1",
    "qumge:deepseek/deepseek-v4-flash",
}
...
def test_trigger_for_a_1m_model_is_the_cap_not_the_128k_guess():
    # Marlo 把上限从 250k 降到 60k（feat/compaction-cap），所以三个窗口都落在上限上。
    assert trigger_tokens(None) == min(int(0.8 * DEFAULT_CONTEXT_WINDOW), DEFAULT_CAP_TOKENS)
    assert trigger_tokens(1_000_000) == DEFAULT_CAP_TOKENS
    assert trigger_tokens(200_000) == DEFAULT_CAP_TOKENS
