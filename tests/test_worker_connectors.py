...
# Marlo：上游这里还有一条 test_request_connector_decline_is_a_plain_outcome，
# 断言 request_connector 走 Inbox 那条 {approved} 路径。我们的 request_connector
# 是一键连接（connect_routes.make_connector_requester，结果键是 connected），
# 由 tests/test_connector_request.py 覆盖；这里只剩 grant_connector 走上游路径。
...
def test_joined_boxes_have_the_reviewer_on_by_default(manager):
    # A desktop keeps the flag's default — Marlo 的默认是开（owner 2026-09-15，
    # config.auto_approve）；a joined box (remote.json present) is on…
    from coworker.config import load_config

    assert manager.auto_approve() is bool(load_config().auto_approve)
...
async def test_staffing_gate_works_without_a_socket(manager):
    ...
    assert engine.team_approver is not None and engine.items_approver is not None and engine.connector_granter is not None
