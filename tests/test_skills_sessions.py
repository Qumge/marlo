...
def test_live_load_skill_semantics(manager):
    ...
    # 上游这里断言的是 loaded["instructions"]。我们把这个键改成了 "reference"，
    # 并且把正文夹在 =BEGIN/END SKILL REFERENCE= 之间 —— 键名是模型读到的第一个
    # 关于"这段文字是什么"的信号，而技能正文来自公开目录（见 skills/base.py）。
    # 断言"正文在里面"而不是"等于正文"，因为外面还包着那层框。
    assert "late body" in loaded["reference"]
    assert "instructions" not in loaded, "正文不能以 instructions 之名交给模型"
    ...
    assert "early" in engine.context_provider()
    assert "early body" in engine.registry.execute("load_skill", {"name": "early"})["reference"]
    ...
    engine.messages.append(
        {
            "role": "tool",
            "tool_call_id": "c1",
            # "reference"，不是 "instructions" —— 这是 load_skill 在我们这边【真实】
            # 返回的形状（见 skills/base.py）。照抄上游的 "instructions" 的话，这条
            # 测试会用一个现实中不存在的消息去验撤销指令，然后一路绿灯，而线上
            # 那个功能其实从来没触发过。
            "content": _json.dumps(
                {"name": "used-one", "reference": "used body", "resources_path": "x"}
            ),
        }
    )
