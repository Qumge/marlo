...
def build_engine(
    *,
    ...
    #
    # persona 自带的技能目录（上游 OPE-58）排在【前面】：后面的目录覆盖前面的，
    # 所以用户全局/工作区里同名的那份会盖掉 bundle 里的。上游是在下面第二次建
    # loader 时传的，我们只有这一个实例，所以在这里传。
    ...
    # could use. Marlo: the tool is OUR one-click connect (tools/connect.py, the same
    # one the `connect` capability ships), not upstream's connreq version — one name,
    # one behaviour, one card. Skip if the capability already registered it.
    if agent.connectors and "request_connector" not in registry.names():
        from .connectors.descriptors import DESCRIPTORS as _CONN_DESCRIPTORS
        from .tools.connect import request_connector_tool

        registry.register(
            request_connector_tool(
                [d.name for d in _CONN_DESCRIPTORS if getattr(d, "available", True)]
            )
        )
