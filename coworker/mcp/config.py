...
Workspace: <workspace>/.coworker/mcp.json   (read only after the user trusts that
           workspace — same gate as repository `allowed_commands` — and **global
           wins** on name clash, so a repo cannot shadow a server you configured)

上游 6217dbc 把优先级从「workspace 覆盖 global」翻成了「global 赢」。对我们不是
中性的：qumge-skills 写在 global，翻转之后一个受信任的仓库也没法用同名条目把它
顶掉。这里跟上游走。
