...
class TurnEngine:
    def __init__(
        ...
        *,
        ...
        #
        # Marlo：上游 2026-09 也做了一个 request_connector，和我们的一键连接同名。
        # 我们的（connector_requester + 设备码 + 卡片队列）留着管 request_connector；
        # 上游那条 Inbox 路径只拿来管 grant_connector，所以改名叫 granter ——
        # 两个同名参数并存就是 SyntaxError，合并时撞上过。
        connector_granter: Optional[
            Callable[[dict[str, Any]], "Awaitable[dict[str, Any]]"]
        ] = None,
    ...
    def _append_notice(self, kind: str, text: Optional[str] = None, **fields: Any) -> None:
        ...
        【两个 field 的语义值得写下来】—— 它们现在顺着 **fields 走，但含义不是任意的：
        ...
        balance endpoint that would otherwise supply it is unavailable. `None` values
        are dropped below, so both stay omitted (never `null`/empty) when absent."""
    ...
    async def _handle_tool_calls(
        ...
        for tool_call in tool_calls:
            ...
            if tool_call.name == "grant_connector":
                async for event in self._handle_connector_grant(tool_call):
                    yield event
                continue
    ...
    async def _handle_connector_grant(self, tool_call: ToolCall) -> AsyncIterator[Event]:
        """`grant_connector` (a lead asks the human to give one of its workers a
        connector) — spec §11.6. Upstream routes `request_connector` here too; Marlo's
        `request_connector` is the one-click connect in `_handle_connector_request`.
        ...
        if not connector or (request == "grant" and not worker):
        ...
        elif self.connector_granter is None:
            ...
            result = await self._wait_tool(tool_call,
                self.connector_granter(dict(args), tool_call.id),
                interrupted={"approved": False, "error": "interrupted by user"},
            ) or {"approved": False, "error": "no response"}
