...
def _browser_page(
    ...
    return (
        ...
        f"<title>{_html.escape(title)} — Marlo</title><style>"
        ...
        "</style></head><body>"
        '<div class="card"><div class="mark"><i></i>Marlo</div>'
        f"{icon}<h1>{_html.escape(title)}</h1><p>{_html.escape(detail)}</p>{err}</div>"
        '<div class="foot">Served locally by Marlo on your Mac</div>'
        "</body></html>"
    )
...
_CONNECT_FAILED_DETAIL = (
    ...
    "Close this tab and try again from Marlo."
)
...
def create_app(manager: SessionManager) -> FastAPI:
    ...
    # ---- qumge 技能目录（我们独有）-------------------------------------------
    # 上游的那组路由管的是【已经在本机的技能】：增删改、作用域、上传。这一组管的是
    # 【货源】—— 从 qumge.com 上 4500 个公开 SKILL.md 里搜一个装进来。两者不重叠：
    # 装进去之后，它就是上游 SkillStore 眼里一个普通的全局技能（同一个目录，
    # folder-is-truth），能在设置里启用/禁用/删除。
    #
    # 对话里 Marlo 自己找技能走的是 MCP（mcp__qumge__search_skills）；这一组是给
    # 想自己看看的用户的第二个入口。
    #
    ...
    # /v1/skills/bundles 与 /v1/skills/bundles/install（技能包）2026-09-14 撤掉 ——
    # 「技能」页只列技能。
    ...
    async def mcp_oauth_callback(
        ...
        if error:
            return HTMLResponse(
                _browser_page(
                    "Sign-in failed",
                    "The service reported an error. Return to Marlo and try again.",
                    ok=False,
                    error=error,
                ),
                status_code=400,
        ...
        if not code or not mcp_oauth.deliver_callback(code, state or None):
            return HTMLResponse(
                _browser_page(
                    ...
                    "The sign-in may have timed out. Return to Marlo and start it again.",
                    ok=False,
                ),
                status_code=400,
            )
        return HTMLResponse(
            _browser_page(
                "Connected",
                "Sign-in complete. You can close this tab and return to Marlo.",
                ok=True,
            )
        )
    ...
    async def cloud_auth_callback(code: str = "", state: str = "", error: str = ""):
        ...
        signin_failed_detail = (
            "Close this tab and try signing in again from Marlo."
        )
        if error:
            return HTMLResponse(
                _browser_page(
        ...
        return HTMLResponse(
            _browser_page(
                ...
                "You can close this tab and return to Marlo.",
            )
        )
    ...
    async def ws_session(ws: WebSocket, session_id: str) -> None:
        ...
        # Marlo：上游这里给 request_connector 和 grant_connector 共用一个 requester，
        ...
        try:
            while True:
                ...
                if kind == "approval":
                    ...
                    # 一个 kind 两种卡：一键连接读 connected，上游 grant_connector
                    # 读 approved。GUI 两个字段都发，这里两个都转，谁要谁拿。
                    _resolve_pending(
                        json.dumps(
                            {
    ...
    # 同一个理由：设备码一键授权的路由住在 connect_routes.py。原本它长在上游的
    # 连接器路由表中间，是一个 66 行的 hunk。
    from .connect_routes import connect_router

    app.include_router(connect_router(manager))
