...
import { showMachines } from "./flags";
...
const SUGGESTION_KEYS: { ico: string; key: ParseKeys }[] = [
...
export function App() {
  ...
  const refreshSessions = useCallback(() => {
    ...
    // Marlo：远程机器默认不露面（flags.showMachines）—— 不取列表，机器分组、
    // 机器选择器、离线机器行都跟着不出现。
    ...
    const handleEvent = (ev: WsEvent) => {
      ...
      switch (ev.type) {
        ...
        case "connector_requested":
          ...
          // worker 授连接器）和我们的（在对话里连一个账号，带设备码）。看 payload 分流。
          if (d.request === "connect" || d.request === "grant") {
  ...
  const respondConnector = (connect: boolean, feedback?: string) => {
    ...
    // 盯着一个什么都没有的界面，不知道该去哪。
    // 上游 §11.6 那种（request 字段）没有浏览器那一趟，回答即终局。
  ...
  if (booting || !uiReady) {
    return (
      <div className={"app boot-splash" + (overlay ? " tauri-overlay" : "")}>
        ...
        {/* The real Marlo mark (6-point star, same as the app/tray icon) — the old
  ...
  return (
    ...
    >
      ...
      <Sidebar
        onManagePersonas={() => setSurface("settings")}
      ...
      {surface === "scheduled" ? (
        ...
        <SkillsView
          onCreateSkill={(description: string, machineId?: string | null) => {
...
// "connected" 是我们的（账号连上了），"approved" 是上游 §11.6 的；两种 connreq 共用 kind。
function resolveLastConnReq(items: Item[], resolved: "connected" | "approved" | "declined"): Item[] {
  const copy = [...items];
  for (let i = copy.length - 1; i >= 0; i--) {
    const it = copy[i];
