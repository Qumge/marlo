...
export async function createSandbox(
  ...
  if (!r.ok) {
    ...
    return { error: typeof detail === "string" ? detail : detail?.message || i18n.t("misc.api.http_error", { status: r.status }) };
  }
...
export async function beginBrowserManagedConnect(
  ...
  const token = apiToken();
  if (!token) return { ok: false, error: i18n.t("sidebar.not_signed_in_row") };
  ...
  if (!r.ok) {
    let detail = i18n.t("misc.api.http_error", { status: r.status });
    try {
      const d = await r.json();
...
export async function connectManagedGrantSealed(
  ...
  if (!res.ok) return { ok: false, error: d.error || d.message || i18n.t("misc.api.http_error", { status: res.status }) };
  return d;
}
...
export async function setScopeRouting(
  ...
  return res.ok ? { ok: true } : { ok: false, error: i18n.t("misc.api.http_error", { status: res.status }) };
}
...
export async function delegateConnection(
  ...
  if (!res.ok) return { ok: false, error: body.detail || body.error || i18n.t("misc.api.http_error", { status: res.status }) };
...
export async function addConfiguration(body: {
  ...
  return { ok: false, error: typeof detail === "string" ? detail : detail?.error || i18n.t("misc.api.http_error", { status: res.status }), held_by: detail?.held_by };
}
...
export async function editConfiguration(
  ...
  if (res.ok) return { ok: true, configuration: data.configuration };
  const detail = data.detail ?? data;
  return { ok: false, error: typeof detail === "string" ? detail : detail?.error || i18n.t("misc.api.http_error", { status: res.status }) };
}
