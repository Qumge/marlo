...
export function scopeNote(
  ...
  if (name.startsWith("mcp__")) {
    if (mcpDest?.transport === "http")
      return {
        text: t("approval.scope.leaves_mac", {
          dest: mcpDest.host || t("approval.scope.mcp_server_fallback"),
        }),
        external: true,
      };
    if (mcpDest?.transport === "stdio")
      return { text: t("approval.scope.mcp_local"), external: false };
    return { text: t("approval.scope.mcp_unknown"), external: true };
  }
  // Egress (§1.9): the request itself reaches the network — never "stays on this computer".
  if (name === "web_fetch")
    return {
      text: t("approval.scope.leaves_mac", { dest: grantHost(args?.url) || t("approval.scope.web_fallback") }),
      external: true,
    };
  if (name === "web_search")
    return {
      text: t("approval.scope.leaves_mac", { dest: t("approval.scope.search_provider_dest") }),
      external: true,
    };
