...
export function TaskRow({
  ...
  const line =
    ...
    item.status ||
    t(("teamview.state_" + item.group) as ParseKeys);
  return (
    <button
      type="button"
        ...
        <table className="team-asks">
          ...
          <tbody>
            {Object.entries(s.asks).map(([kind, a]) => (
              <tr key={kind}>
                <th>{t(("teamview.ask_" + kind) as ParseKeys)}</th>
                <td>{a.answered}</td>
                <td>{a.waiting}</td>
              </tr>
            ))}
                  ...
                  !!tasks.length && (
                    <section key={group}>
                      <h3 className="team-group-heading">
                        {t(("teamview.state_" + group) as ParseKeys)}
                      </h3>
                      {tasks.map((item) => (
            ...
            <>
              ...
              {GROUPS.map((group) => {
                ...
                return group === "done" ? (
                  ...
                  <section key={group}>
                    <h3 className="team-group-heading">
                      {t(("teamview.state_" + group) as ParseKeys)} <span>{tasks.length}</span>
                    </h3>
                    {rows}
                  </section>
                );
