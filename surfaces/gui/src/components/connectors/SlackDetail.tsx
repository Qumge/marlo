...
function PersonPicker({
      ...
      getSlackDirectory(teamId || "default", q)
        .then((r) => {
          if (r.ok) {
            setRows(r.members || []);
            setErr(null);
          } else setErr(r.error || t("slack.directory_unavailable"));
        })
        .catch(() => setErr(t("slack.directory_unavailable")));
    }, 200);
  ...
  const pick = async (m: SlackMember) => {
    ...
    if (result?.ok === false) {
      setErr(result.error || t("misc.slack.add_person_failed"));
      return;
    }
    setOpen(false);
    setQ("");
  ...
  return (
    <span className="relative" ref={wrap}>
      <button
        ...
        title={t("slack.pick_from_directory")}
        onClick={toggle}
      >
        {buttonLabel ?? t("slack.add_person")}
      </button>
      {open && (
        <div
        ...
        >
          <input
            ...
            placeholder={t("slack.type_a_name")}
            value={q}
          ...
          <div className="max-h-56 overflow-y-auto py-1">
            {err ? (
              ...
              <div className="px-2 py-1.5 text-meta text-faint">{t("slack.no_matches")}</div>
            ) : (
              candidates.map((m) => (
                <button
                  key={m.id}
