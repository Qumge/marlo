    ...
    <div className="sub-chip-wrap" ref={ref}>
      ...
      {open && (
        <div className="sub-pop" onMouseDown={(e) => e.stopPropagation()}>
          <div className="sub-pop-head">{t("uiChannelsListened")}</div>
          {channels.length === 0 ? (
            <div className="dim sub-pop-empty">{t("uiNoChannels")}</div>
          ) : (
            channels.map((c) => {
              ...
              <div className="sub-pop-row" key={c}>
                ...
                <button className="sub-pop-x" title={t("uiUnsubscribe")} onClick={() => remove(c)}>
                  ×
                </button>
              </div>
              );
