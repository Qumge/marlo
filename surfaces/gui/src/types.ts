...
export type Item =
  ...
  | {
      ...
      server?: string;
      detail?: string;
      // `cause` narrows an error WITHOUT changing kind —— GUI 据此给出针对性的动作
      // （余额不足时的充值按钮）。折进 kind 会让 retry() 的「尾部是不是 error notice」
      // 守卫失效，而这条错误恰恰是最该能重试的那种。
      cause?: string;
      // `topup_url`：402 响应体自带的充值链接。刷新之后按钮还要能点，所以它必须
      // 跟着 notice 一起持久化 —— 余额接口不可用时它就是唯一的权威来源。
      topup_url?: string;
    }
