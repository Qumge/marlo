...
const TAB_BY_SLUG: Record<string, string> = {
  ...
  account: "account",
  models: "models",
  // Marlo：上游的 context 页并进了「模型」（压缩/Token 节省卡片在那里）；
  // skills 在我们这里是独立页面（账号菜单 ▸ 技能），不在设置里 —— 老书签落到
  // 能看到东西的地方，而不是一页空白。
  context: "models",
  memory: "memory",
  coworkers: "personas",
...
// 别名（context → models）只进不出：反查时跳过，否则模型页的 URL 会被写成 #/settings/context。
const SLUG_ALIASES = new Set(["context"]);
const SLUG_BY_TAB: Record<string, string> = Object.fromEntries(
  Object.entries(TAB_BY_SLUG)
    .filter(([slug]) => !SLUG_ALIASES.has(slug))
    .map(([slug, tab]) => [tab, slug]),
);
