...
test("skills-settings: create via the menu → name-first banner; edit persists", async ({ page }) => {
  ...
  // getByTitle 是【子串】匹配，而我们的余额小条 title 写着
  // "Qumge credit — click to add more" —— cr·edit 命中 "Edit"，且它在 DOM 里排在
  // 技能行前面，于是 .first() 点开的是账号菜单，编辑面板压根没打开（排查时先怀疑
  // 是 i18n transform，关掉它照样失败，才找到这里）。
  // exact: true 只认真正的 title="Edit"。上游没有余额小条，所以那边不会撞。
