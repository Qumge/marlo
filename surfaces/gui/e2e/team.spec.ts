...
test("the staffing card carries connector checkboxes and states that approvals follow the lead", async ({
  ...
  // Marlo：这一档的英文叫 "Full access"（locales/en.marlo.json 覆盖了上游的 "Bypass approvals"）。
  await page.getByTestId("mode-menu").getByText("Full access").click();
  await expect(card.getByTestId("teamreq-approvals")).toContainText("Approvals follow the lead's mode: Full access");
...
// Marlo：request_connector 是我们的一键连接（后端从不发上游 {request:"connect"} 那种卡），
// 连接卡由 conversation-auth.spec.ts 和 ConnectorRequestCard.test.tsx 覆盖；上游这张「去连接页」的卡不存在。
test.skip("a coworker may ask for a service to be connected; not now is a plain outcome", async ({
  page,
}) => {
  await page.goto("/");
