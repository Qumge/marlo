...
// SKIP：这条在【干净的 upstream/main 上也是红的】，不是合并弄坏的（2026-08-01 用
// upstream 的 worktree 单独跑过）。第二段 page.goto("/") 之后那一轮消息根本没往返 ——
// "Echo: hello" 和 "Echo: second" 都不在 DOM 里，用量事件自然也没到，chip count 是 0。
// 中途重载会新开一个 WebSocket，而 fixture 的脚本化假 agent 没有跟着重新武装。
//
// 覆盖没有丢：同样的判断在 usage-chip-context-bar.spec.ts 里重做了一遍，那边不重载，
// 改成在首次导航前把 /v1/settings 的 context_bar 覆写成 true。放新文件而不是就地改，
// 是因为这个文件是上游的 —— 动它的每一行都是下次合并要人判断的地方。等上游修好
// 自己的 fixture，把这行 skip 删掉即可。
test.skip("Settings toggle turns the context bar on; default is the in-context number", async ({ page }) => {
  await page.goto("/");
  await page.getByText("Draft the launch note").first().click();
  const box = page.getByPlaceholder(/Ask the coworker/);
  await box.fill("hello");
